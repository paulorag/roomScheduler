...
export default function MyBookings() {
    ...
    const [downloadingIcsId, setDownloadingIcsId] = useState<number | null>(null);
    ...
    async function handleDownloadIcs(id: number) {
        setDownloadingIcsId(id);
        setError("");
        try {
            await api.bookings.downloadIcs(id);
        } catch (err) {
            if (err instanceof ApiError) {
                setError(err.message);
            } else {
                setError("Erro ao exportar convite iCalendar.");
            }
        } finally {
                ...
                <div className="space-y-4">
                    {bookings.length === 0 ? (
                        ...
                        bookings.map((booking) => {
                            ...
                            const isDownloading = downloadingIcsId === booking.id;
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2 w-full md:w-auto justify-end">
                                        <button
                                            ...
                                            className="px-3 py-2 bg-slate-50 border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-100 transition text-sm font-medium flex items-center gap-1 shadow-sm cursor-pointer disabled:opacity-60"
                                            title="Baixar convite para Google Agenda / Outlook"
                                        >
                                            📅 {isDownloading ? "Baixando..." : ".ICS"}
                                        </button>
